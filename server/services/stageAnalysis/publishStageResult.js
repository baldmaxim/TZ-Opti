...
//
// Раньше движок публиковал стадию четырьмя независимыми шагами (issues + статус
// одной транзакцией, затем сигналы своей, затем сегменты, затем активация), и
// падение между ними оставляло стадию «успешной» без опубликованного снимка.
...
//
// ЕДИНСТВЕННЫЙ production-путь публикации: stageAnalysisEngine.runStageWork
// зовёт этот сервис в одной db.transaction после успешного завершения LLM.
// Здесь материализация снимка, его активация И перевод стадии в
...
// прежний движок коммитил 'reviewing' ПЕРВЫМ, до сигналов и до перевода
// указателя, поэтому падение в середине оставляло стадию «успешной» без
