...
Разбираются **все листы** книги (лист без шапки «Вопрос/Ответ» помечается непригодным
и пропускается); при применении можно выбрать нужные листы. Импортёр сам находит
строку заголовков и распознаёт колонки по ключевым словам (регистр не важен, порядок
свободный). Распознаются:
...
**Импорт идёт РАУНДАМИ, а не полной заменой** (`qa_imports`: номер раунда + дата +
статус `pending|applied|discarded`). Основной путь UI двухфазный: предпросмотр diff
против текущих записей (`POST …/qa/imports/preview` — новые / изменённые ответы /
без изменений) → применение выбранных листов (`…/apply`) или отмена (`…/discard`).
Прежние вопросы и ответы **не удаляются**: записи несут статус
`active | superseded | cancelled`; изменившийся ответ вставляется новой записью с
явной ссылкой на отменяемую (`supersedes_entry_id`), совпавшие пары не дублируются
(разметка инженера — пункт ТЗ, контуры — сохраняется). `cancelled` инженер ставит
и снимает вручную; в анализ (стадия 2, авто-привязка) идут только `active`.
Одношаговый путь (загрузка документа в слот `qa`, спасательный реимпорт перед
стадией 2) применяет все листы сразу и благодаря diff идемпотентен.

> **Характеристики** — это отдельный стандартный шаблон таблицы (`characteristicsTemplate.js`), который инженер заполняет на вкладке «Таблица характеристик». Из Q&A xlsx они **не** импортируются, и **импорт Q&A таблицу характеристик не трогает никогда** (инвариант закреплён тестом `qaImport.integration.test.js`; прежний `DELETE FROM characteristics` в импортёре был реликтом MVP-схемы и удалён).
...
**Результат части и история её выполнения — разные вещи и лежат в разных таблицах.** `analysis_segments` — **кэш**, скоупленный ревизией документов (ключ: тендер + стадия + `document_revision_id` + номер части; переиспользование только при совпадении `input_hash` и `config_version`, поэтому новая версия ТЗ не затирает кэш прошлой). `analysis_run_segments` — **неизменяемая история выполнения**: одна строка на (прогон + часть), пишет только прогон-владелец и только пока он идёт; видно `status` (`pending|running|completed|failed|interrupted|skipped`), `source` (`llm` — считалось моделью, `cache`/`checkpoint` — переиспользовано), попытки, ошибку и времена. Отсюда три следствия: повтор стадии не переспрашивает модель про уже посчитанные части; упавшую часть можно **пересчитать точечно** — `POST /api/tenders/:id/stages/:n/segments/:idx/retry` (гасит кэш этой части, историю не трогает); и **прогон не затирает прошлый** — видно, на какой части встал вчерашний анализ (`GET …/segments?run_id=`, debug-страница `/tenders/:id/debug/segments` с переключателем прогонов).
...
| **Checkpoint** | Стадия сохраняет каждую посчитанную часть ТЗ дважды: в `analysis_tasks.checkpoint_json` (в пределах задачи) и в `analysis_segments` (кэш по ревизии, переживает задание, сверка по `input_hash` + `config_version`). Повтор после падения **не переспрашивает LLM** про уже сделанное, а одну часть можно пересчитать точечно. Конвейер чекпойнтится шагами: успешный шаг не пересобирается. |
| **Один прогон на задание** | `analysis_run` создаётся при постановке стадии в очередь (`status='running'`, реальные `started_at`, ревизия документов, версия конфигурации) и его id идёт в задание и во все части ТЗ. Повторная попытка задачи продолжает **тот же** прогон, а не заводит второй снимок. |
...
| **После рестарта** | Осиротевшие задачи либо **продолжаются** (возврат в очередь + чекпойнт), либо получают статус **`interrupted`**, когда попытки исчерпаны, — и задание, и стадия чинятся: `stageN_status` возвращается в `open`, а **начатый прогон завершается тем же статусом** (`failed` / `cancelled` / `interrupted`) с реальным `finished_at` и причиной в `summary`. Отдельной «фиктивной» failed-строки не создаётся. Прогон, оставшийся `running` без единого живого задания, закрывается как `interrupted` на старте сервера и воркера. |
