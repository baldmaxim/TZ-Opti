...
// LEGACY-слой сборки итога по issues (fallback с этапа 7; основной путь —
// clusterReviewService: issue_clusters → review_decisions.cluster_id).
// Из находок стадий делает ОДИН результат: находки, указывающие на одно и то же
// место ТЗ (тот же абзац + перекрытие символьных диапазонов; фолбэк — точная
// цитата), группируются. В группе выбирается primary (по критичности; тай-брейк —
// порядок стадий, затем char_start), прочие — related. Конфликт помечается
// (решает инженер — ничего не теряем). dedupeExportDecisions переиспользует
// issue-fallback docx-экспорта (на одно место — решение primary).
