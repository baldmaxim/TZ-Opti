...
function applyOne(decision, paragraphs, commentsDoc, docDoc, { author, date }) {
  ...
  try {
    if (visual.docx === 'del+ins') {
      ...
      const newText = resolveRedaction(issue, decision);
      if (newText) {
        applyInsertion(target, delEl, newText, { id: nextTrackChangeId(docDoc), author, date, propsFromRun: propsRun });
      }
    } else {
    ...
    return { ...base, status: 'applied', visual: visual.docx, fallbackUsed: false };
  } catch (err) {
