...
└── server/    Express + pg (Postgres) + Multer
    ...
    └── db/
        ...
        ├── migrate.js
        ├── seed.js
        ├── standardRisks.js                          — библиотека типовых рисков (ядро R01–R15 + группы ЖК-тендера R16+)
...
| Манифест тендерного пакета: у каждого документа — редакция, статус актуальности (`actual`/`informational`/`superseded`), дата, приоритет при противоречии, применимость (корпус/раздел), цепочка замены; вход анализа выбирается по манифесту, а не «последний загруженный»; несколько актуальных ВОР (по корпусам) сосуществуют; правка манифеста = новая ревизия набора документов | ✅ Реализовано |
| Большой ВОР идёт в анализ ПАКЕТАМИ (а не выбрасывается по лимиту символов); «нет в ВОР» — только по пересечению всех пакетов (устойчивому к длине цитаты); готовый вход сопоставления ТЗ ↔ ВОР ↔ чек-лист | ✅ Реализовано |
| Структурное сопоставление ТЗ↔ВОР: требование → позиции ведомости → количество/единица (факт ведомости) → включённые/недостающие операции → исключения → примечания по единицам/коэффициентам → уверенность → подтверждение инженера; частичное покрытие («перегородки есть, усиления/закладные — нет») — отдельный тип `учтено_частично`; страница «Покрытие ВОР» | ✅ Реализовано |
| Стадии 1–4 — LLM-агенты по тексту ТЗ (Чек-лист+ВОР, Q&A+характеристики, Условия компании, Риски) | ✅ Реализовано |
| Покрытие существенных условий (Стадия 3): статус каждой темы (соответствует / противоречит / отсутствует / неоднозначно + инженерские «в другом документе» / «проверка договора» / «неприменимо» / «урегулировано в договоре» / «риск принят»), 24 договорные темы покрытия (включая ПНР, страхование, расторжение, давальческие, BIM, права на документацию), «условие отсутствует» — безъякорная находка с действием (запрос / допущение / условие КП / проверка договора / резерв); находку гасит только ЗАКРЫВАЮЩИЙ статус инженера — примечание или «проверка договора» тему не закрывают, риск остаётся в реестре; комментарий инженера в панели покрытия | ✅ Реализовано |
| Библиотека типовых рисков реального ЖК-тендера: ядро R01–R15 + R16–R39 по 23 группам (границы работ, цена, доп. работы, фронт, логистика, материалы, номинированные, ИД, приёмка, оплата, штрафы/лимиты, страхование, гарантия, ПНР, непредвиденные, расторжение, приоритет документов, городские ограничения, BIM, конфиденциальность); условия «Работы вне Договора» и «К вниманию Заказчика» заполнены защитными формулировками | ✅ Реализовано |
| Конвейер анализа: signals → draft_issues → critic → clustering → self-analysis (Стадия 5 = QC над итогом) | ✅ Реализовано |
| Стадия 5 разделена: QC качества кластеров (основание / последствие / рекомендация / дубли / конфликты / завышенная критичность, каждый вывод привязан к кластеру) + независимый CHALLENGER-шаг конвейера — читает ТЗ против собранного итога, пропуски публикуются снимком стадии 5 (сигналы `challenger`) и пересборкой слоёв становятся обычными замечаниями с решением инженера | ✅ Реализовано |
...
| Жёсткий запрет экспорта незавершённой рецензии: `reviewReadinessService` (решённость рабочего списка, carry-over, свежесть снимка) → 409 `REVIEW_NOT_READY` для docx/CSV/JSON/MD и создания/активации согласованной версии; legacy issue-путь — только явным `?source=issues`, авто-fallback по пустым решениям убран; `GET …/review/readiness` + блокировка на странице «Экспорт» | ✅ Реализовано |
