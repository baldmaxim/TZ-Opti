...
const ALLOWED_SECTIONS = ['checklist', 'conditions', 'risks', 'qa', 'documents', 'characteristics'];

async function getLocksMap(tenderId) {
  const rows = await db.queryAll('SELECT section, locked_at FROM setup_locks WHERE tender_id = ?', tenderId);
  const out = {};
...
exports.list = async (req, res) => {
  ...
  res.json({ locks: await getLocksMap(req.params.id) });
};
...
exports.lock = async (req, res) => {
  ...
  res.json({ ok: true, locks: await getLocksMap(req.params.id) });
};

exports.unlock = async (req, res) => {
  await ensureTender(req.params.id);
  ensureSection(req.params.section);
  await db.queryRun('DELETE FROM setup_locks WHERE tender_id = ? AND section = ?', req.params.id, req.params.section);
  res.json({ ok: true, locks: await getLocksMap(req.params.id) });
};
